                  ...
                  <TableCell key={dayIndex} className="text-center">
                    <ul>
                      {example_json_entries[part][dayIndex]?.map((entry, index) => (
                        <Link to={routes.user({id: parseInt(entry.name.split(' ')[1])})} key={index}>
                          <li className="flex flex-col items-center space-x-4 rounded-md p-2 hover:bg-accent">
                            <div className={`
                              flex h-6 w-6 items-center justify-center rounded-md
                              ${entry.preference === 'NECESSARY' ? 'text-green-500' :
                                entry.preference === 'PREFERRED' ? 'text-green-300' :
                                entry.preference === 'DESIRED' ? 'text-green-300' :
                                  entry.preference === 'NEUTRAL' ? 'text-yellow-300' :
                                    entry.preference === 'UNDESIRED' ? 'text-orange-400' :
                                      'text-red-500'}
                            `}>
